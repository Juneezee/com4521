...
int mouse_buttons = 0;
float rotate_x = 0.0f, rotate_z = 0.0f;
...
void initHistVertexData() {
    ...
    for (unsigned int x = 0; x < D; x++) {
        for (unsigned int y = 0; y < D; y++) {
            const unsigned int offset = (x + y * D) * 3 * 4;
    ...
    for (unsigned int x = 0; x < D; x++) {
        for (unsigned int y = 0; y < D; y++) {
            const unsigned int index = x + y * D;
    ...
    //map instance
    glVertexAttribIPointer((GLuint)vs_hist_instance_index, 1, GL_UNSIGNED_INT, 0, 0);
    // Set up instance id attributes pointer in shader
    ...
    glBufferData(GL_TEXTURE_BUFFER, D * D * 1 * sizeof(float), 0, GL_DYNAMIC_DRAW);
    // 1 float elements in a texture buffer object for histogram density
...
void initNBodyVertexData() {
    ...
    //map instance
    glVertexAttribIPointer((GLuint)vs_nbody_instance_index, 1, GL_UNSIGNED_INT, 0, 0);
    // Set up instance id attributes pointer in shader
    ...
    glBufferData(GL_TEXTURE_BUFFER, N * 2 * sizeof(float), 0, GL_DYNAMIC_DRAW);
    // 2 float elements in a texture buffer object for x and y position
