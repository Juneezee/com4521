...
__global__ void maximumMark_atomic_kernel(student_records *d_records) {
    ...
    const float mark = d_records->assignment_marks[idx];
    const int id = d_records->student_ids[idx];
...
__global__ void maximumMark_shuffle_kernel(student_records *d_records, student_records *d_reduced_records) {
    ...
    for (int offset = 16; offset > 0; offset >>= 1) {
        const float shuffle_mark = __shfl_down(assignment_mark, offset);
        const int shuffle_id = __shfl_down(student_id, offset);
    ...
    if (threadIdx.x % 32 == 0) {
        const unsigned int o_index = idx / 32;
