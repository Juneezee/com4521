...
__global__ void image_blur(uchar4 *image, uchar4 *image_output) {
    ...
    const int output_offset = x + y * blockDim.x * gridDim.x;
    uchar4 pixel;
    ...
    for (int i = -SAMPLE_SIZE; i <= SAMPLE_SIZE; i++) {
        for (int j = -SAMPLE_SIZE; j <= SAMPLE_SIZE; j++) {
            ...
            if (y_offset >= IMAGE_DIM)
                y_offset -= IMAGE_DIM;
            const int offset = x_offset + y_offset * blockDim.x * gridDim.x;
            pixel = image[offset];
...
__global__ void image_blur_texture2D(uchar4 *image_output) {
    ...
    for (int i = -SAMPLE_SIZE; i <= SAMPLE_SIZE; i++) {
        for (int j = -SAMPLE_SIZE; j <= SAMPLE_SIZE; j++) {
            const int x_offset = x + i;
            const int y_offset = y + j;
            //2D texture lookup
...
int main(void) {
    ...
    // texture1D version
    cudaBindTexture(nullptr, sample1D, d_image, image_size);
    ...
    // texture2D version
    cudaBindTexture2D(nullptr, sample2D, d_image, desc, IMAGE_DIM, IMAGE_DIM, IMAGE_DIM * sizeof(uchar4));
