...
__global__ void simple_instance_kernel(float4 *instance_data, unsigned int width, unsigned int height, float time) {
    ...
    const float red = x / static_cast<float>(width);
    const float green = y / static_cast<float>(height);
    const float blue = 0.0f;

    // Exercise 1.6) Create a displacement value
    const float freq = 4.0f;
    const float H = 0.1f;
    const float displacement = sinf(red * freq + time) * cosf(green * freq + time) * H;
...
void setVertexData(float *verts) {
    ...
    const float quad_width = 1.0f / static_cast<float>(GRID_WIDTH);
    const float quad_height = 1.0f / static_cast<float>(GRID_HEIGHT);

    for (int x = 0; x < GRID_WIDTH; x++) {
        for (int y = 0; y < GRID_HEIGHT; y++) {
            const int offset = (x + y * GRID_WIDTH) * 3 * 4;

            const float x_min = static_cast<float>(x) / static_cast<float>(GRID_WIDTH);
            const float y_min = static_cast<float>(y) / static_cast<float>(GRID_HEIGHT);
...
void setVertexInstanceData(unsigned int *vertex_instance_ids) {
    for (int x = 0; x < GRID_WIDTH; x++) {
        for (int y = 0; y < GRID_HEIGHT; y++) {
            const int index = x + y * GRID_WIDTH;
