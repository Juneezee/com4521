...
static double iterations_d[HEIGHT][WIDTH];
//store for the escape time as a double (with fractional part) for NIC method only
static int histogram[MAX_ITERATIONS + 1]; //histogram if escape times
static int local_histogram[HEIGHT][MAX_ITERATIONS + 1];
//only required for exercise 2.2.3, HEIGHT is the maximum possible number of threads that could be initialised as it the maximum size of the number of work units (width of parallel loop)
...
#pragma omp parallel for private(i, x, c_r, c_i, n_r, n_i, o_r, o_i, mu) if(hist_method != OMP_SERIAL) schedule(dynamic, 2)
    for (y = 0; y < HEIGHT; y++) {
        for (x = 0; x < WIDTH; x++) {
