...
__device__ float sphere_intersect(Sphere *s, float ox, float oy, float *n) {
    const float dx = ox - s->x;
    const float dy = oy - s->y;
    const float radius = s->radius;
    if (dx * dx + dy * dy < radius * radius) {
        const float dz = sqrtf(radius * radius - dx * dx - dy * dy);
...
__global__ void ray_trace(uchar4 *image, Sphere *d_s) {
    ...
    const float ox = x - IMAGE_DIM / 2.0f;
    const float oy = y - IMAGE_DIM / 2.0f;
...
__global__ void ray_trace_read_only(uchar4 *image, Sphere const *__restrict__ d_s) {
    ...
    for (int i = 0; i < d_sphere_count; i++) {
        ...
        float n;
        const float t = sphere_intersect_read_only(s, ox, oy, &n);
        if (t > maxz) {
            const float fscale = n;
            r = s->r * fscale;
            g = s->g * fscale;
